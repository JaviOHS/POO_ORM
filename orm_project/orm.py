...
def probar_orm():
    ...
    def basic_queries():
        ...
        notes_8 = NoteDetail.objects.filter(
            note1__gt=8.0
        )  # __gt: Busca los registros con nota1 mayor que 8.0. | __gte: Mayor o igual.
        print_message(notes_8)

        title("5. Seleccionar todas las notas con nota2 menor que 9.0:")
        notes_9 = NoteDetail.objects.filter(
            note2__lt=9.0
        )  # __lt: Busca los registros con nota2 menor que 9.0. | __lte: Menor o igual.
    ...
    def conditionals_queries():
        ...
        notes_2 = NoteDetail.objects.filter(
            Q(recuperation=None) | Q(note2__gt=9.0) # Se puede colocar: Q(recuperation__isnull=True).
        )
    ...
    def date_queries():
        ...
        notes = NoteDetail.objects.filter(created_at__year=2024) # __year: Busca los registros del año 2024.
        print_message(notes)

        title("16. Seleccionar todas las notas creadas en el último mes:")
        last_month = timezone.now() - timezone.timedelta(days=30)
        notes_2 = NoteDetail.objects.filter(created_at__gte=last_month)  # __gte: Mayor o igual.
        print_message(notes_2)

        title("17. Seleccionar todas las notas creadas en el último dia:")
        last_day = timezone.now() - timezone.timedelta(days=1)
        notes_3 = NoteDetail.objects.filter(created_at__gte=last_day) # __gte: Mayor o igual.
        print_message(notes_3)

        title("18. Seleccionar todas las notas creadas antes del 2023:")
        notes_4 = NoteDetail.objects.filter(created_at__year__lt=2023) #__lt: Menor que.
        print_message(notes_4)

        title("19. Seleccionar todas las notas creadas en marzo de cualquier año:")
        march_notes = NoteDetail.objects.filter(created_at__month=3) # __month: Busca los registros del mes 3.
    ...
    def advances_queries():
        ...
        students = Student.objects.annotate( # annotate: Para agregar un campo calculado.
            name_length=Length("name") # Length: Para obtener la longitud del campo.
        ).filter( 
            name_length=10 # Se filtra por la longitud del campo.
        )
        print_message(students)
        ...
        notes_2 = NoteDetail.objects.filter(
            ~Q(recuperation=None) & Q(note1__gt=F("note2"))  # Se puede directamente con (recuperation__isnull=False). | F: Para referenciar el campo.
        )
        print_message(notes_2)
        ...
        notes_4 = NoteDetail.objects.filter(
            recuperation__gt=F("note1") + F("note2")  # Se suman los campos.
        )
        print_message(notes_4)
    ...
    def subqueries_annotations():
        ...
        students_recuperation = Student.objects.annotate( # annotate: Para agregar un campo calculado.
        ...
        notes_in_list = NoteDetail.objects.filter(note1__in=note_list) # note1__in: Busca los registros con note1 en la lista.
        ...
        [print(f"-ID: {note.id} {note}") for note in specific_notes] if specific_notes else print(f"{reset_color}No hay registros.")
        print(purple_color + "------" * 15 + reset_color)
        ...
        def calculate_notes(student):
            ...
            # aggregate: Para realizar operaciones de agregación. | Sum: Para sumar los valores de un campo. | output_field: Para especificar el tipo de campo.
        ...
        def max_note(student):
            ...
            note_max = NoteDetail.objects.filter(student=student).aggregate(
                max_note1=Max("note1"), max_note2=Max("note2"), max_recuperation=Max("recuperation"),
            )
            max_note_values = [
                note_max.get("max_note1", 0), note_max.get("max_note2", 0), note_max.get("max_recuperation", 0),
            ]
        ...
        def min_note(student):
            ...
            note_min = NoteDetail.objects.filter(student=student).aggregate(
                min_note1=Min("note1"), min_note2=Min("note2"), min_recuperation=Min("recuperation"),
            )
            min_note_values = [
                note_min.get("min_note1", 0), note_min.get("min_note2", 0), note_min.get("min_recuperation", 0),
            ]
        ...
        def average_notes(student):
            ...
            try:
                student_detail = NoteDetail.objects.filter(student=student)
            except NoteDetail.DoesNotExist:
                print("El estudiante especificado no tiene notas registradas.")
                return

            if not student_detail.exists():
                print("El estudiante especificado no tiene notas registradas.")
                return
            student_notes = NoteDetail.objects.filter(student=student).aggregate(
                average_note1=Avg("note1"), average_note2=Avg("note2") # Avg: Para obtener el promedio de los valores de un campo.
            )
            total = round(
    ...
    def subqueries_inverse_relationships():
        ...
        def specific_period():
            ...
            if not details_note.exists():
                print(
                    "No hay detalles de notas para las notas del período especificado."
                )
                return
        ...
        def specific_teacher():
            ...
            notes = Note.objects.filter(
                teacher=teacher
            )  # Se puede hacer directamente filtrando: teacher__id=10, teacher__name="...", teacher__dni="...".
            if not notes.exists():
        ...
        def notes_specific_student():
            ...
            try:
                ...
                return
            notes = NoteDetail.objects.filter(
                student=student, note1__gt=7.0, note2__gt=7.0
            )
            if not notes.exists():
        ...
        def period_notes_students_order():
            ...
            try:
                ...
                print("El estudiante especificado no existe.")
                return
            notes = NoteDetail.objects.filter(student=student).order_by( # order_by: Para ordenar los registros.
                "note__period__start_date"
            )
            if not notes.exists():
                print("El estudiante no tiene notas registradas.")
        ...
        def average_notes_period():
            ...
            if not notes.exists():
                print(
                    f"El estudiante {student.name} no tiene notas registradas en el período {period.start_date} - {period.end_date}."
                )
                return
            ...
            total_recuperation = (
                notes.aggregate(total_recuperation=Sum("recuperation"))["total_recuperation"] or 0
            )
            average_note = (total_note1 + total_note2 + total_recuperation) / (
                total_notes * 3
            )
        ...
        def student_notes_by_subject(student_id):
            ...
            try:
                ...
                print("El estudiante especificado no existe.")
                return
            notes = NoteDetail.objects.filter(student=student).order_by(
                "note__subject__description"
            )
            if not notes.exists():
    ...
    def sentences_update():
        def sentences_update_note1(nota_new):
            ...
            if not notes.exists():
                ...
                return
            notes.update(note1=nota_new) # update: Para actualizar los registros.
        ...
        def update_obserbation(mensaje):
            ...
            aprobados = NoteDetail.objects.filter(
                Q(note1__gte=50, note2__gte=20) | Q(note1__gte=20, note2__gte=50) | Q(note1__gte=50, recuperation__gte=20) | Q(note2__gte=50, recuperation__gte=20)  # Aprobados por las 4 condiciones
            )
            if not aprobados.exists():
                print("No hay estudiantes aprobados.")
            else:
                # Actualizar la observación para los estudiantes aprobados
                updated_count = aprobados.update(observation=mensaje)
        ...
        def update_notas_period():
            ...
            try:
                ...
                notes = NoteDetail.objects.filter(
                    created_at__date__range=(period.start_date, period.end_date)
                )
    ...
    def sentences_delete():
        def sentences_delete_student(student_id):
            ...
            try:
                student = Student.objects.get(pk=student_id) # Obtener el estudiante por su ID
                print(f"Estudiante encontrado: {student.name}")
                notas_a_eliminar = NoteDetail.objects.filter(student=student) # Filtrar y contar las notas del estudiante
                count_eliminadas = notas_a_eliminar.count()
                if count_eliminadas > 0:
                    # Eliminar las notas
                    notas_a_eliminar.delete() # delete: Para eliminar los registros.
        ...
        def sentences_delete_student_state(student_id):
            ...
            try:
                student = Student.objects.get(pk=student_id) # Obtener el estudiante por su ID
                print(f"Estudiante encontrado: {student.name}")
                notas = NoteDetail.objects.filter(student=student) # Filtrar las notas del estudiant
                print(f"Notas encontradas: {notas.count()}")
                updated_count = notas.update(state=False) # Actualizar el campo 'state' a False para eliminar lógicamente las notas
                print(f"Se actualizó la observación de {updated_count} registros.")
            except Student.DoesNotExist:
                print(f"No se encontró ningún estudiante con ID {student_id}.")
            except Exception as e:
        ...
        def sentences_delete_notes(periodo_id):
            ...
            try:
                periodo = Period.objects.get(pk=periodo_id) # Obtener el período por su ID
                print(f"Período encontrado: {periodo.start_date} - {periodo.end_date}")
                notas_en_periodo = NoteDetail.objects.filter(note__period=periodo) # Filtrar las notas que pertenecen a ese período
                notas_count = notas_en_periodo.count()
                notas_en_periodo.delete() # Eliminar físicamente las notas
        ...
        def sentences_delete_period(periodo_id):
            ...
            try:
                ...
                notas_en_periodo = NoteDetail.objects.filter(note__period=periodo) # Filtrar las notas que pertenecen a ese período
                notas_count = notas_en_periodo.count()
                notas_en_periodo.update(state=False) # Eliminar lógicamente las notas (actualizar el campo state a False)
    ...
    def create_student_note():
        ...
        try:
            ...
            note = Note.objects.create(
                teacher=teacher, subject=subject, period=period, user=teacher.user
            )
